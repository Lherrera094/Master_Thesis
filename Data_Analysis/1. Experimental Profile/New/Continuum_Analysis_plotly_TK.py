...
def alfv_continuum_read(x,f_max,path):
    ...
    color = ["white","black","navy","darkslategrey","sandybrown","blue","darkturquoise",
             ...
             "grey","red","dodgerblue","mediumvioletred","black"]
...
# %%
def alfv_continuum(f_max,save_file,path,profile):
    # plot the data
    fig = go.Figure()
    cont = [3,5,6,7,9,10,11,13,14,15,17]
    #cont = [3,5,6,7,9,10,11,13,14,15]
    for i in cont:
    ...
    fig.update_layout(
        title={
            'text': f"Alfvén Continuum Experimental Profile",
            'y': 0.91,  # Adjust the vertical position of the title
            'x': 0.5,  # Set the x position to center the title
            'xanchor': 'center',
            'yanchor': 'top',
            'font': dict(size=30, color='black')  # Customize the font size of the title
        },
        showlegend=False,
    ...
    fig.update_xaxes(title_text='r/a',title_font = {"size": 25},color = 'black')
    fig.update_yaxes(title_text='Frequency (kHz)',title_font = {"size": 25},color='black')
    fig.update_layout(yaxis_range=[0,f_max])    
    fig.show()
    
    fig.write_html(f"{save_file}/{profile}_Continuum.html")
    fig.write_image(f"{save_file}/{profile}_continuum.png",width=800, height=600, scale=3)
...
def individual_helical_plot(df,f_max,dict_fam,path,save_file,fam,image_path):
    ...
    mode = []
    for i in df['dominant_mode']:
        mode.append(change_modes_order(i))
    
    df['dominant_mode'] = mode
    
    fig = px.scatter(df,
                     x = "radial_pos_maximum",
                     y = "f(kHz)",
                     text = df["dominant_mode"],
                     size = df["width"]*1000,
                     size_max=45,
                     color = "Growth Rate",
                     symbol = "Alfvén_mode",
                     hover_data=["efast","beta"],
