...
def read_df(x,prof,fam):
    col_del = ['Unnamed: 0',"dominant_mode",'width_i', 'width_f','coupling', 'radial_pos_2','Possible_EAE',
               'radial_pos_3','Frequency']
...
def alfv_continuum_read(x,f_max,path):
    df = pd.read_csv(f"{path}n = {x}.txt", sep="\t")
    df.columns = ["r",f"n={x}","m","n"]
    df["r"] = np.sqrt(df["r"]) 
...
# %%
def group_helical_plot(df1,df2,df3,f_max,dict_fam,path,save_file,fam):
    ...
    # Create a list of marker shapes based on the 'category' column
    marker_shapes_1 = [shape_map[category] for category in df1['Alfvén_mode']]
    marker_shapes_2 = [shape_map[category] for category in df2['Alfvén_mode']]
    marker_shapes_3 = [shape_map[category] for category in df3['Alfvén_mode']]
    
    #Start Figure
    fig = go.Figure()
    fig.add_trace(go.Scatter(
                         x = df1["radial_pos_maximum"],
                         y = df1["f(kHz)"],
                         mode='markers',
                         opacity = 0.9,
                         hovertemplate='Energy: %{text}<br>X: %{x}<br>Y: %{y}',
                         marker=dict(
                                size=30,
                                color='red',
                                symbol=marker_shapes_1,
                        ),
                        name = 'Experimental'
                     )
                   )
    # Add category text to the hovertemplate
    fig.data[0].text = df1['efast']
    
    fig.add_trace(go.Scatter(
                         x = df2["radial_pos_maximum"],
                         y = df2["f(kHz)"],
                         mode='markers',
                         opacity = 0.7,
                         hovertemplate='Energy: %{text}<br>X: %{x}<br>Y: %{y}',
                         marker=dict(
                                size=30,
                                color='purple',
                                symbol=marker_shapes_2,
                        ),
                        name = 'On-Axis'
                     )
                   )
    #fig.data[0].text = df2['efast']
    
    fig.add_trace(go.Scatter(
                         x = df3["radial_pos_maximum"],
                         y = df3["f(kHz)"],
                         mode='markers',
                         opacity = 0.7,
                         hovertemplate='Energy: %{text}<br>X: %{x}<br>Y: %{y}',
                         marker=dict(
                                size=30,
                                color='steelblue',
                                symbol=marker_shapes_3,
                        ),
                        name = 'Off-Axis'
                     )
                   )
    #fig.data[0].text = df3['efast']

    # Register the on_selection event callback function
    ...
    for i in dict_fam["continuum"]:
        ...
        fig.add_trace(go.Scatter(x = df_cont["r"],
                                       y = df_cont[f"n={i}"], 
                                       mode='markers',
                                        marker=dict(
                                        size=3,
    ...
    fig.show()
    fig.write_image(f"{save_file}/Axis_Displacement_{title}.png",width=800, height=600, scale=3)
    fig.write_html(f"{save_file}/Axis_Displacement_{title}.html")


# %%
# Define the on_selection event callback function
def on_selection(trace, points, selector):
    selected_categories = [df['Alfvén_mode'][index] for index in points.point_inds]
    print("Selected categories:", selected_categories)


# %%
def group_helical_plot2(df1,df2,df3,f_max,dict_fam,path,save_file,fam):
    
    title = dict_fam["title"]
    
    #Start Figure
    fig = go.Figure()
    fig.add_trace(px.scatter(df1,
    ...
    fig.write_html(f"{save_file}/Axis_Displacement_profile_{title}.html")
